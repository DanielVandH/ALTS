...
################################################################################
## ALTS_CV(model_formula, data, maxIters = 20, tol = 1e-4, sigma_p = 0.25, num_folds = 5, num_q = 30)
##  Fit model using ALTS, with q chosen using cross-validation.
##
## Arguments:
##  model_formula: The formula for the regression.
...
##
## Outputs:
##  The output is a list containing the following (same as in `ALTS`):
##    $p: The vector of p iterates.
##    $sigma: The vector of sigma iterates.
##    $beta: The final estimate of the regression coefficients.
##    $model: The final model.
...
ALTS_CV = function(model_formula, data, maxIters = 30, tol = 1e-2, sigma_p = 0.25, num_folds = 5, num_q = 30) {
  ...
  resp = data[, resp_name]
  N = nrow(data)
  fold =  createFolds(resp, k = num_folds, returnTrain = TRUE) # https://github.com/topepo/caret/issues/1193
  mape_results = matrix(nrow = num_q, ncol = num_folds)
  # Start by fitting a model to the data from each fold
  residual_initial = list()
  training_data = list()
  testing_data = list()
  for (j in seq_along(fold)) {
    training_fold = fold[[j]] %>% unique 
    testing_fold = (1:N)[-training_fold]
    training_data[[j]] = data[training_fold, ]
    testing_data[[j]] = data[testing_fold, ]
    residual_initial[[j]] = lad(model_formula, data = training_data[[j]])$residuals
  }
  # Now fit all the models
  for (i in seq_along(q)) {
    for (j in seq_along(fold)) {
      # Fit the model 
      model_fit = ALTS(model_formula, training_data[[j]], q = q[i], 
                       maxIters = maxIters, tol = tol, sigma_p = sigma_p,
                       residual_initial = residual_initial[[j]])
      # Predict from the model
      model_pred = predict(model_fit$model, testing_data[[j]])
      # Error measure 
      mape_results[i, j] = MAPE(testing_data[[j]][, resp_name], model_pred)
    }
  }
  # Now go over each parameter q and average over the errors
  averaged_mapes = rowMeans(mape_results)
  # Select best
...
lpALTS = list(type = "Regression",
              library = c("EnvStats", "L1pack", "dplyr", "quantreg"),
              loop = NULL, 
              prob = NULL,
              levels = NULL)
...
altsGrid = function(x, y, len = NULL, search = "grid") {
  if (search == "grid") {
    out = data.frame(q = seq(0.8, 1.5, length.out = len))
  } else {
    out = data.frame(q = runif(len, min = 0.8, max = 1.5))
  }
  return(out)
}
