...
##
##  7. sim_fnc_1_cv(beta, X, p, sigma_1, sigma_2, num_q = 30, num_folds = 5)
##          Perform a single simulation for the simulation study for the new ALTS using cross-validation.
##
##  8. main_sim_fnc_cv(num_sims, beta, X, p, sigma_1, sigma_2)
##          For a single `p`, perform the cross-validation simulation study for `num_sims` simulations.
##
##  9. complete_sim_fnc_cv(num_sims, beta, X, p, sigma_1, sigma_2) 
##          For each `p` in the vector `p`, perform the simulation study for `num_sims` simulations.
library(EnvStats)
library(L1pack)
library(dplyr)
...
################################################################################
## sim_fnc_1_cv(beta, X, p, sigma_1, sigma_2, num_q = 30, num_folds = 5)
##  Perform a single simulation for the simulation study for the new ALTS using 
##  cross-validation.
##
## Arguments:
##  beta: Known regression coefficients.
...
##  sigma_1: Standard deviation of the clean data.
##  sigma_2: Standard deviation of the contaminated data.
##  num_q = 15: Number of q values to test.
##  num_folds = 5: Number of folds to use so that we use `num_folds`-fold cross-validation. 
##
## Outputs:
##  The output is the result from `ALTS` with the cross-validated `q`, given a list
##  with the single element `$New` for these results.
################################################################################
sim_fnc_1_cv = function(beta, X, p, sigma_1, sigma_2, num_q = 15, num_folds = 5) {
  ## Add noise to the data 
  nobs = nrow(X)
  mu = beta[1] + X %*% beta[2:length(beta)]
  h = floor(p * nobs)
  ...
  model_formula = y ~ x1 + x2 + x3
  ## Fit the results 
  new_results = ALTS_CV(model_formula, data, num_folds = num_folds, num_q = num_q)
  ## Return results 
  return(list(New = new_results))
}

################################################################################
## main_sim_fnc_cv(num_sims, beta, X, p, sigma_1, sigma_2) 
##  For a single `p`, perform the cross-validation simulation study for `num_sims` simulations.
##
## Arguments:
##  num_sims: Number of simulations to perform.
...
##      $rmape: Mean MAPE values from Bacher's ALTS.
################################################################################
main_sim_fnc_cv = function(num_sims, beta, X, p, sigma_1, sigma_2) {
  results = list()
  i = 1
  while (i <= num_sims) {
    tryit = try(sim_fnc_1_cv(beta, X, p, sigma_1, sigma_2), silent = FALSE)
    if (inherits(tryit, "try-error")) {
      print("Error occurred. Restarting loop.")
    } else {
      results[[i]] = tryit
  ...
  sim_results = process_results(results, beta, X, include_bacher = FALSE)
  return(sim_results)
}

################################################################################
## complete_sim_fnc_cv(num_sims, beta, X, p, sigma_1, sigma_2) 
##  For each `p` in the vector `p`, perform the simulation study for `num_sims` simulations.
##
## Arguments:
##  num_sims: Number of simulations to perform.
...
##      $rmape: Mean MAPE values from Bacher's ALTS at each true `p`.
################################################################################
complete_sim_fnc_cv = function(num_sims, beta, X, p, sigma_1, sigma_2) {
  new_q = matrix(data = NA, nrow = 1, ncol = length(p))
  new_p = matrix(data = NA, nrow = 1, ncol = length(p))
  new_sigma = matrix(data = NA, nrow = 1, ncol = length(p))
  new_rmape = matrix(data = NA, nrow = 1, ncol = length(p))
  for (j in 1:length(p)) {
    results = main_sim_fnc_cv(num_sims, beta, X, p[j], sigma_1, sigma_2)
    new_q[1, j] = results$New$q
    new_p[1, j] = results$New$p
    new_sigma[1, j] = results$New$sigma
    new_rmape[1, j] = results$New$rmape
